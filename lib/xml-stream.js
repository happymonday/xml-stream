...
function makeEncoder(encoding) {
  // TODO: commented out to remove Iconv dep
  // that was causing issues with webpack.
  // re-implement using something like iconv-lite
  // if support for non utf8 XML files is needed
...
function parse() {
  ...
  var parseChunk = function (data) {
    ...
    if (!xml.parse(data, false)) {
      self.emit(
        "error",
        new Error(xml.getError() + " in line " + xml.getCurrentLineNumber())
      );
    }
  };
